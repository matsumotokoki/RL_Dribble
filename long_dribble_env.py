...
class Dribble_Env(object):
    def __init__(self):
        self.model = load_model_from_path("./xml/world4.xml") 
    ...
    def check_done(self):
        ...
        if ball_x < -95 and -35 < ball_y < 35:
            return True
        else:
            return False

    def check_fall(self):
        ball_z = self.sim.data.body_xpos[2][2]
        robot_z = self.sim.data.body_xpos[1][2]
        if (ball_z < -2) or (robot_z < -2):
            return True
        else:
            return False
