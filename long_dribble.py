import numpy as np
from long_dribble_env import Dribble_Env
...
def reward_val(t,done,fall,dist):
    if done:
        reward = 100
    elif fall:
        reward = -dist * 0.1
...
for episode in range(num_episodes):
    ...
    for t in range(max_number_of_steps):
        ...
        goal_distance = math.sqrt((ball_state[0] + 100)**2 + ball_state[1]**2)
        next_state = np.reshape(next_state,[1,6])
        done = env.check_done()
        fall = env.check_fall()

        reward = reward_val(t,done,fall,goal_distance)
        ...
        if done or fall or t >= 199:
            if max_step < t:
                max_step = t
            total_reward_vec = np.hstack((total_reward_vec[1:], reward))
            ball_state = env.get_state()[6:8]
            print('{:4d} Episode finished, {:3d} steps, reward: {:6.2f}, ave: {:6.2f}, x: {:6.2f}, y: {:6.2f}, dist: {:5.2f}'\
                    .format(episode+1,t+1,reward,total_reward_vec.mean(),ball_state[0],ball_state[1],180-goal_distance),flush=True)
            break
